...
public class IndustrialMightCondition extends BaseMarketConditionPlugin {
    ...
    public void apply(String id) {
        ...
        if (market.isInEconomy() && market.getFaction().isShowInIntelTab()) {
            AoTDFactionResearchManager manager = AoTDMainResearchManager.getInstance()
                    .getSpecificFactionManager(this.market.getFaction());
            if (manager == null)
                AoTDMainResearchManager.getInstance().addNewFactionIfNotPresent(market.getFaction());
            manager = AoTDMainResearchManager.getInstance().getSpecificFactionManager(this.market.getFaction());
            if (manager == null)
                return;
            if (AoTDMainResearchManager.getInstance().getSpecificFactionManager(this.market.getFaction())
                    .haveResearched("aotd_tech_advanced_logistic")) {
                handleBonuses();
            }
        }
        if (market.isPlayerOwned() || market.getFaction().isPlayerFaction()) {
            ...
            if (manager == null)
                return;
            if (AoTDMainResearchManager.getInstance().getManagerForPlayer()
                    .haveResearched("aotd_tech_advanced_logistic")) {
                handleBonuses();
            }
        }
    ...
    public void handleBonuses() {
        if (hasTwoIndustriesForSynergy(AoTDIndustries.TERMINUS, AoTDIndustries.MILITARY_HEAVY)) {
            market.getStats().getDynamic().getMod(Stats.GROUND_DEFENSES_MOD).modifyMult("IndSynergyMiliHeavy", 1.5f,
                    "Presence of Military Industrial Complex (Placeholder)");

        } else {
            market.getStats().getDynamic().getMod(Stats.GROUND_DEFENSES_MOD).unmodifyMult("IndSynergyMiliHeavy");
        }
        ...
        if (hasTwoIndustriesForSynergy(AoTDIndustries.BENEFICATION, AoTDIndustries.POLICRYSTALIZATOR)) {
            ...
            poli.getSupply(AoTDCommodities.REFINED_METAL).getQuantity().modifyFlat("IndSynergyCascade", +1,
                    "Industry Synergy");
        ...
        if (hasTwoIndustriesForSynergy(AoTDIndustries.SUBLIMATION, AoTDIndustries.CASCADE_REPROCESSOR)) {
            ...
            cascade.getSupply(AoTDCommodities.PURIFIED_TRANSPLUTONICS).getQuantity().modifyFlat("IndSynergyPoli", +1,
                    "Industry Synergy");
        } else {
            Industry cascade = market.getIndustry(AoTDIndustries.CASCADE_REPROCESSOR);
            if (cascade != null) {
        ...
        if (hasTwoIndustriesForSynergy(AoTDIndustries.CASCADE_REPROCESSOR, AoTDIndustries.ORBITAL_SKUNKWORK)) {
            ...
            for (MutableCommodityQuantity mutableCommodityQuantity : triHeavy.getAllSupply()) {
                triHeavy.getSupply(mutableCommodityQuantity.getCommodityId()).getQuantity().modifyFlat("IndSynergyTri",
                        +1, "Industry Synergy");
            }
            ...
            if (triHeavy != null) {
                for (MutableCommodityQuantity mutableCommodityQuantity : triHeavy.getAllSupply()) {
                    triHeavy.getSupply(mutableCommodityQuantity.getCommodityId()).getQuantity()
                            .unmodifyFlat("IndSynergyTri");
                }
            }
        }
        ...
        if (hasTwoIndustriesForSynergy(AoTDIndustries.POLICRYSTALIZATOR, AoTDIndustries.HEGEMONY_HEAVY)) {
            ...
            for (MutableCommodityQuantity mutableCommodityQuantity : hegeHeavy.getAllSupply()) {
                hegeHeavy.getSupply(mutableCommodityQuantity.getCommodityId()).getQuantity()
                        .modifyFlat("IndSynergyHege", +1, "Industry Synergy");
            }
        } else {
            Industry hegeHeavy = market.getIndustry(AoTDIndustries.HEGEMONY_HEAVY);
            if (hegeHeavy != null) {
    ...
    protected void createTooltipAfterDescription(TooltipMakerAPI tooltip, boolean expanded) {
        ...
        tooltip.addPara("Current Industry Synergies", Color.ORANGE, 10f);

        if (hasTwoIndustriesForSynergy(AoTDIndustries.TERMINUS, AoTDIndustries.MILITARY_HEAVY)) {
            tooltip.addPara("Terminus and Militarized Heavy Industry: %s", 10f, Misc.getStoryBrightColor(),
                    "1.5 bonus to Military garrisons");

        }
        if (hasTwoIndustriesForSynergy(AoTDIndustries.TERMINUS, AoTDIndustries.SUPPLY_HEAVY)) {
            tooltip.addPara("Terminus and Civilian Heavy Production: %s", 10f, Misc.getStoryBrightColor(),
                    "+1 stability");

        }
        if (hasTwoIndustriesForSynergy(AoTDIndustries.BENEFICATION, AoTDIndustries.POLICRYSTALIZATOR)) {
            tooltip.addPara("Policrystalizator and Benefication: %s", 10f, Misc.getStoryBrightColor(),
                    "Decreases upkeep of Policrystalizator by 25% and increases Policrystalizator's production by 1 unit.");

        }
        if (hasTwoIndustriesForSynergy(AoTDIndustries.SUBLIMATION, AoTDIndustries.CASCADE_REPROCESSOR)) {
            tooltip.addPara("Cascade Reprocessor and Sublimation : %s ", 10f, Misc.getStoryBrightColor(),
                    "Decreases upkeep of Cascade Reprocessor by 25% and increases Cascade Reprocessor's production by 1 unit.");

        }
        if (hasTwoIndustriesForSynergy(AoTDIndustries.CASCADE_REPROCESSOR, AoTDIndustries.ORBITAL_SKUNKWORK)) {
            tooltip.addPara("Cascade Reprocessor and Orbital Skunkwork Facility: %s ", 10f, Misc.getStoryBrightColor(),
                    "Increases Orbital Skunkwork Facility production by 1 unit.");

        }
        if (hasTwoIndustriesForSynergy(AoTDIndustries.POLICRYSTALIZATOR, AoTDIndustries.HEGEMONY_HEAVY)) {
            tooltip.addPara("Policrystalizator and Orbital Fleetwork Facility: %s ", 10f, Misc.getStoryBrightColor(),
        ...
        if (hasTwoIndustriesForSynergy(AoTDIndustries.CONSUMER_INDUSTRY, AoTDIndustries.RESORT)) {
            tooltip.addPara("Consumer Industry and Resort Center: %s ", 10f, Misc.getStoryBrightColor(),
                    "Increases Resort income by 10%");

        }
        if (hasTwoIndustriesForSynergy(AoTDIndustries.ARTISANAL_FARMING, AoTDIndustries.RESORT)) {
            tooltip.addPara("Artisanal Farming and Resort Center: %s ", 10f, Misc.getStoryBrightColor(),
                    "Increases Artisanal Farming's income by 10%");

        }
        if (hasTwoIndustriesForSynergy(AoTDIndustries.UNDERWORLD, AoTDIndustries.RESORT)) {
            tooltip.addPara("Underworld and Resort Center: %s ", 10f, Misc.getStoryBrightColor(),
                    "Increases planet income by 5% ");

        }
        if (hasTwoIndustriesForSynergy(AoTDIndustries.TERMINUS, AoTDIndustries.RESORT)) {
            tooltip.addPara("Terminus and Resort Center: %s ", 10f, Misc.getStoryBrightColor(),
    ...
    public boolean showIcon() {
        ...
        //
        return firstarg && AoTDMainResearchManager.getInstance().getSpecificFactionManager(market.getFaction())
                .haveResearched("aotd_tech_advanced_logistic");
