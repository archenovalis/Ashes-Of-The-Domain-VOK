...
public class UpgradeListUI implements CustomDialogDelegate {
    ...
    public boolean isCryovolcanicOrFrozen(MarketAPI market) {
        ...
        if (market.getPlanetEntity() != null) {
            if (market.getPlanetEntity().getTypeId().equals("frozen")
                    || market.getPlanetEntity().getTypeId().equals("cryovolcanic")
                    || market.getPlanetEntity().getTypeId().equals("frozen1")) {
    ...
    public void createCustomDialog(CustomPanelAPI panel, CustomDialogCallback callback) {
        ...
        for (String industryId : upgrades) {
            CustomPanelAPI mainPanel = Global.getSettings().createCustom(ENTRY_WIDTH, ENTRY_HEIGHT + 2f,
                    new ButtonReportingCustomPanel(this));
            ...
            BuildingMenuMisc.createTooltipForIndustry((BaseIndustry) ind, Industry.IndustryTooltipMode.UPGRADE,
                    indTooltipTest, true, false, widthSub + 10, true, false, false);
            TooltipMakerAPI indTooltip = indPanel.createUIElement(indPanel.getPosition().getWidth(),
                    indTooltipTest.getHeightSoFar(), false);
            BuildingMenuMisc.createTooltipForIndustry((BaseIndustry) ind, Industry.IndustryTooltipMode.UPGRADE,
                    indTooltip, true, false, widthSub + 10, true, false, false);
            ...
            tooltipImage.addCustom(indPanel, 0f).getPosition().inTL(label.getPosition().getX(),
                    -label.getPosition().getY() + 5);
            mainPanel.getPosition().setSize(ENTRY_WIDTH, indTooltip.getHeightSoFar() + 35);
            TooltipMakerAPI tooltipButton = mainPanel.createUIElement(ENTRY_WIDTH, mainPanel.getPosition().getHeight(),
                    false);
            ButtonAPI button = tooltipButton.addAreaCheckbox("", industryId,
                    industry.getMarket().getFaction().getBaseUIColor(),
                    industry.getMarket().getFaction().getDarkUIColor(),
                    industry.getMarket().getFaction().getBrightUIColor(), ENTRY_WIDTH,
                    mainPanel.getPosition().getHeight(), 0f);
